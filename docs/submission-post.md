...
| 6:30 | Local validation green; **I click Publish in Studio Web** (Solution publish requires a user-context Auth0 token — see Path-B note below); subsequent version bumps replay via `UiPathClient.publish_maestro_project` with the captured Studio Web request shape |
...
| **D3** | No documented public CLI verb publishes a Studio Web Solution. Built `UiPathClient.publish_maestro_project` against the captured Studio Web `POST /studio_/backend/api/Solution/{solution_id}/Publish-Requests` endpoint (real capture committed in `b294a53`, replacing the synthetic fixture) plus a Playwright UI fallback. **The empirically-confirmed constraint:** Studio Web mutations require user-scope tokens (authorization-code flow); the client-credentials grant used by AURORA's headless daemon can only obtain `OR.*` Orchestrator scopes — proven by probing every documented Studio Web/Solutions/PM scope on the live tenant 2026-05-12, all rejected with `invalid_scope`. The submission ships **Path B**: humans publish the initial Solution via Studio Web (a single click), and AURORA orchestrates everything else — start instances, complete tasks, rotate assets, correlate webhooks — via OR.* scopes. **Path A** (interactive-login refresh-token bootstrap → user-scope tokens rotated on the VPS, ~150 LOC) is scaffolded behind a planned `aurora-login` slash command and on the post-submission roadmap. | `lib/aurora/uipath_client.py`, `lib/aurora/playwright/{capture,publish_ui_fallback}.py`, `tests/fixtures/maestro/publish_request.json` (captured), `docs/maestro-publish-bridge.md` (T-D3) |
